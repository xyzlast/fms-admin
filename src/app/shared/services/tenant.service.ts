...
export class TenantService {
  ...
  getDeviceTree(tenantId): Promise<DeviceTree> {
    const url = '/fms-api/v2/admin/common/devices';
    const params = new URLSearchParams();
    params.set('tenantId', tenantId);
    const obj = this.http.get(url, { search: params, withCredentials: true });
    return BaseService.progress(obj).then(mapping => {
      return new DeviceTree(mapping);
    });
  }
}
